...
mkValidator p deadline () ctx =
    ...
    traceIfFalse "deadline not reached"            checkAfterDeadline
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    checkSig :: Bool
    checkSig = p `elem` txInfoSignatories info

    checkAfterDeadline :: Bool
    checkAfterDeadline = from deadline `contains` txInfoValidRange info
