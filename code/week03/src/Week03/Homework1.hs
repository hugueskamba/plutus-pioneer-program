...
mkValidator dat () ctx
    | checkBeforeDeadline == true = traceIfFalse "You are not the beneficiary" (checkSig (beneficiary1 dat))
    | otherwise = traceIfFalse "You are no longer the beneficiary" (checkSig (beneficiary2 dat))
  where
      info :: TxInfo
